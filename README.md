...
| `FINMATE_LLM_MODEL_FAST` | – | Model rẻ hơn cho việc vặt (phân loại chat, đoán danh mục tin nhắn ngân hàng). Trống = dùng model chính |
...
- **Đồng hồ token và tiền**: thẻ khoá AI hiện số lượt gọi, token vào/ra, phần đọc từ bộ đệm, và **ước tính tiền tháng này** theo bảng giá niêm yết của Anthropic (bản ghi 2026-06-24, trong `server/src/services/chat/gia.js`). Model không có trong bảng giá thì app nói thẳng là chưa quy ra tiền được — không bịa một con số để người dùng lập kế hoạch chi tiêu theo nó. Đồng hồ nằm trong sổ riêng từng người nên sống qua mỗi lần khởi động lại và không lẫn giữa người này với người kia.
- **Model cho việc vặt** (`FINMATE_LLM_MODEL_FAST`, hoặc ô trong Cài đặt): phân loại câu chat và đoán danh mục cho tin nhắn ngân hàng là việc ngắn, khuôn mẫu, làm hàng chục lần mỗi ngày. Chọn `claude-haiku-4-5` cho việc này rẻ hơn nhiều lần so với model chính. **App không tự hạ model** — bỏ trống thì mọi thứ vẫn chạy bằng model chính.
